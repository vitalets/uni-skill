export function concatWithSeparator<
  ...
  T2 extends string | undefined
  >(s1: T1, s2: T2, separator: string) {
  return [ s1, s2 ]
    .filter(text => typeof text === 'string' && text.length > 0)
    .join(separator);
}

export function concatWithNewline<
  T1 extends string | undefined,
  T2 extends string | undefined
  >(s1: T1, s2: T2) {
...
export function stripSpeakTags<T extends string | undefined>(s: T) {
...
export function stripEmoji<T extends string | undefined>(s: T) {
